...
/* Freie Bearbeitungen: Grundpreis + Zuschlag je laufendem Meter Schnittkante.
   ...
   Die Ausklinkung startet niedriger, weil der Fraeser von aussen anfaehrt und
   nicht eintauchen muss. */
const FREI_PRICE  = {basis:21.9, lfm:10.8};
const NOTCH_PRICE = {basis:14.9, lfm:10.8};
const PRESETS = {
...
const S = { mat:'dekor', dekor:'buk', mpxSurface:'natur', absColor:'dekor',
            ...
            extras:{bohr:false,custom:false,kanal:false}, kanal:{l:100,w:60,pos:'mitte'},
...
function isStandard(){
  if(S.extras.custom||S.extras.kanal||isLack()||S.form==='szwal'||S.form==='lform'||cornerCount()>0||S.cuts.some(c=>!c.preset)) return false;
...
/* Freie Ausschnitte, Ausklinkungen und Konturen haben seit v1.7.0 einen
   Sofortpreis nach Formel — nur die eigene Skizze und der Kabelkanal (Tiefe
   und Laufmeterpreis noch offen) gehen weiter ins Angebot. */
function needsOffer(){ return S.extras.custom||S.extras.kanal||isLack(); }
function delivDate(){
...
/* ═══════ 2D-SVG ═══════ */
function drawStage(){
  /* PAD war 76: die Platte nutzte nur ~75 % der Flaeche, das Produktbild wirkte
     winzig neben dem Kantenfoto. 58 laesst den Massketten weiter genug Luft. */
  const svg=$('stage'); const W=600,H=444,PAD=58;
  ...
  if(S.form==='round'){
    ...
    const NO=notchOutline(x,y,pw,ph,R4,sc);
    const pdRect=NO?NO.d:roundPath(x,y,pw,ph,R4);
    ...
    const seg=NO?NO.segs:edgeSegments(x,y,pw,ph,R4);
    seg.forEach((dd,i)=>{
    ...
    if(S.form==='rect'){
      [[x,y],[x+pw,y],[x+pw,y+ph],[x,y+ph]].forEach((pt,i)=>{
        if(eckeWeg(i)) return;      /* Ecke ist ausgeklinkt, dort gibt es nichts zu runden */
    ...
    /* Kabelkanal (von unten gefraest -> gestrichelt, halbtransparent) */
    if(S.extras.kanal){
      const kl=Math.min(+S.kanal.l||0, S.form==='rect'?(S.kanal.pos==='quer'?d.h:d.w):d.w)*sc;
      const kw=Math.max(4,(+S.kanal.w||60)/10*sc);
      let kx,ky,kW,kH;
      if(S.kanal.pos==='quer'){ kW=kw; kH=kl; kx=x+pw/2-kw/2; ky=y+(ph-kl)/2; }
      else { kW=kl; kH=kw; kx=x+(pw-kl)/2; ky=S.kanal.pos==='hinten'? y+Math.max(6*sc,10) : y+ph/2-kw/2; }
      inner+=`<rect x="${kx}" y="${ky}" width="${kW}" height="${kH}" rx="${Math.min(6,kw/3)}"
        fill="#F2F0EB" fill-opacity=".55" stroke="#00000055" stroke-dasharray="7 4"/>`;
      inner+=`<text class="dim-text" x="${kx+kW/2}" y="${ky+kH/2+4}" text-anchor="middle" style="font-size:11px">Kanal ${S.kanal.l} cm × ${S.kanal.w} mm</text>`;
    }
    /* Ausschnitte */
    if(S.extras.bohr){
  ...
  if(S.form!=='round'){
    ...
    all.forEach((c2,i)=>{
      ...
      if(c2.t==='n'){ return; }        /* Ausklinkung steckt schon in der Kontur */
      if(c2.t==='p'){
...
function addNotch(){
  if(S.form!=='rect'){ toast('Ausklinkungen aktuell nur beim Rechteck'); return; }
  const d=dims();
  const w=Math.max(5,Math.min(20,Math.round(d.w*0.2)));
  const dp=Math.max(3,Math.min(15,Math.round(d.h*0.25)));
  S.cuts.push({t:'n', e:0, a:0, w, dp, r:10});
  toast('Ausklinkung hinzugefügt — Kante und Maße stellst du unten ein');
  render();
  const box=$('cutList'); if(box) box.lastElementChild?.scrollIntoView({behavior:'smooth',block:'center'});
}
...
/* Absolute Punkte einer freien Kontur (cm) */
...
function fmtCut(c){
  if(c.t==='n') return `Ausklinkung ${cutMass(c)}`;
...
function cutShort(c){
  ...
  if(c.t==='n') return z(c.w)+' × '+z(c.dp)+' cm · '+KANTEN_NAMEN[c.e];
...
function dimV(x,y1,y2,t){return `<line class="dim-line" x1="${x}" y1="${y1}" x2="${x}" y2="${y2}"/>
  ...
  <text class="dim-text" x="${x+15}" y="${(y1+y2)/2}" text-anchor="middle"
    transform="rotate(-90 ${x+15} ${(y1+y2)/2})">${t}</text>`}
...
/* ── Ausklinkungen: die Plattenkontur wird umgebaut ──────────────────────────
   Eine Ausklinkung haengt an einer Kante (e = 0 hinten, 1 rechts, 2 vorne,
   3 links), sitzt im Abstand a vom Kantenanfang, ist w breit und dp tief.
   Liegt sie buendig am Kantenanfang oder -ende, verschwindet die Plattenecke
   und es entsteht der klassische Eckausschnitt. */
const EDGE_DIR=[[1,0],[0,1],[-1,0],[0,-1]];   /* Richtung entlang der Kante */
const EDGE_IN =[[0,1],[-1,0],[0,-1],[1,0]];   /* Richtung in die Platte hinein */
function notchList(){ return S.form==='rect' ? S.cuts.filter(c=>c.t==='n') : []; }
/* Konturpunkte in Plattenkoordinaten (cm, x nach rechts, y nach hinten→vorne).
   rad: Zahl in cm, oder {k:i} als Platzhalter fuer den Radius der Plattenecke i.
   tag: zu welcher Kante die Linie AB diesem Punkt gehoert. */
function outlineData(W,H){
  const C=[[0,0],[W,0],[W,H],[0,H]], ns=notchList();
  const pts=[], rad=[], tag=[];
  const kante=e=>(e%2===0)?W:H;
  const buendigAnfang=e=>ns.some(c=>c.e===e&&c.a<=0.05);
  const buendigEnde=e=>ns.some(c=>c.e===e&&(c.a+c.w)>=kante(e)-0.05);
  for(let e=0;e<4;e++){
    const len=kante(e), A=C[e], dir=EDGE_DIR[e], inn=EDGE_IN[e];
    const P=(t2,dd)=>[A[0]+dir[0]*t2+inn[0]*dd, A[1]+dir[1]*t2+inn[1]*dd];
    if(!buendigAnfang(e) && !buendigEnde((e+3)%4)){ pts.push(C[e]); rad.push({k:e}); tag.push(e); }
    ns.filter(c=>c.e===e).sort((p,q)=>p.a-q.a).forEach(c=>{
      const a0=Math.max(0,Math.min(len,c.a)), a1=Math.max(a0,Math.min(len,c.a+c.w));
      const rr=Math.max(0,(c.r||0))/10;                      /* mm -> cm */
      if(a0>0.05){ pts.push(P(a0,0)); rad.push(0); tag.push(e);
                   pts.push(P(a0,c.dp)); rad.push(rr); tag.push(e); }
      else { pts.push(P(a0,c.dp)); rad.push(0); tag.push(e); }
      if(a1<len-0.05){ pts.push(P(a1,c.dp)); rad.push(rr); tag.push(e);
                       pts.push(P(a1,0)); rad.push(0); tag.push(e); }
      else { pts.push(P(a1,c.dp)); rad.push(0); tag.push(e); }
    });
  }
  return {pts,rad,tag};
}
/* Wie roundPoly, liefert zusaetzlich je Kante einen eigenen Pfad, damit die
   Kantenfarben und das Anklicken der Kanten erhalten bleiben. */
function roundPolyTagged(pts, rad, tag){
  const n=pts.length, t=[];
  for(let i=0;i<n;i++){
    const p=pts[i], a=pts[(i-1+n)%n], b=pts[(i+1)%n];
    const v1=[a[0]-p[0],a[1]-p[1]], v2=[b[0]-p[0],b[1]-p[1]];
    const l1=Math.hypot(v1[0],v1[1])||1, l2=Math.hypot(v2[0],v2[1])||1;
    const rr=Math.min(Math.max(0,rad[i]||0), l1/2, l2/2);
    t.push({p1:[p[0]+v1[0]/l1*rr, p[1]+v1[1]/l1*rr],
            p2:[p[0]+v2[0]/l2*rr, p[1]+v2[1]/l2*rr], rr,
            sweep:(v1[0]*v2[1]-v1[1]*v2[0])<0?1:0});
  }
  let d=`M ${t[0].p2}`;
  const segs=['','','',''];
  for(let i=0;i<n;i++){
    const cur=t[i], nx=t[(i+1)%n];
    const bogen = nx.rr>0.5 ? ` A ${nx.rr} ${nx.rr} 0 0 ${nx.sweep} ${nx.p2}` : ` L ${nx.p2}`;
    d += ` L ${nx.p1}` + bogen;
    segs[tag[i]|0] += ` M ${cur.p2} L ${nx.p1}` + (nx.rr>0.5?bogen:'');
  }
  return {d:d+' Z', segs};
}
/* Ist die Plattenecke i durch eine Ausklinkung verschwunden? */
function eckeWeg(i){
  const d=dims();
  return notchList().some(c=>{
    const len=(c.e%2===0)?d.w:d.h;
    return (c.a<=0.05 && c.e===i) || ((c.a+c.w)>=len-0.05 && ((c.e+1)%4)===i);
  });
}
function notchOutline(x,y,pw,ph,R4,sc){
  if(!notchList().length) return null;
  const d=dims(), od=outlineData(d.w,d.h);
  const pts=od.pts.map(p=>[x+p[0]*sc, y+p[1]*sc]);
  const rad=od.rad.map(r=>(r&&typeof r==='object')?R4[r.k]:r*sc);
  return roundPolyTagged(pts, rad, od.tag);
}
...
function addCutHoles(sh){
  ...
  S.cuts.forEach(c2=>{
    if(c2.t==='n') return;                       /* steckt schon in der Aussenkontur */
...
function plateShape(){
  ...
  const x=-w/2,y=-h/2;
  /* Ausklinkungen: dieselbe Kontur wie in der Draufsicht, nur gespiegelt
     (die 3D-Geometrie wird beim Kippen um X gedreht). */
  if(notchList().length){
    const od=outlineData(d.w,d.h);
    const P=od.pts.map(p=>[-w/2+p[0]/10, h/2-p[1]/10]);
    const R=od.rad.map(r=>(r&&typeof r==='object')?Math.min(cornerR(r.k)/100,cap):r/10);
    polyToShape(sh,P,R);
    addCutHoles(sh);
    return sh;
  }
...
function render(){
  ...
  if(S.extras.kanal)rows.push([`Kabelkanal ${S.kanal.l} cm`,'im Angebot']);
  ...
  buildQuick(); drawStage(); updateMini(); updateSticky(); updateBottomBar(); buildCounts(); buildConf(); syncURL();
}
...
function extrasList(){
  ...
  if(S.extras.kanal) out.push({t:`Kabelkanal ${S.kanal.l} cm`, s:5, warn:true});
...
const KANTEN_NAMEN=['hinten','rechts','vorne','links'];
...
function buildCutList(){
  ...
  box.innerHTML=S.cuts.map((c,i)=>{
    ...
    if(c.t==='c') f+=feld('Durchmesser','d',c.d*10,'mm','min="10"');
      ...
      f+=`<label>Kante<span class="in"><select data-f="e">`+
         KANTEN_NAMEN.map((n,k)=>`<option value="${k}"${k===c.e?' selected':''}>${n}</option>`).join('')+
         `</select></span></label>`;
      f+=feld('Breite','w',c.w*10,'mm','min="20"')+feld('Tiefe','dp',c.dp*10,'mm','min="10"');
      f+=feld('Abstand','a',c.a*10,'mm','min="0"');
    }
    if(c.t==='r'||c.t==='n'||c.t==='p') f+=feld('Radius','r',c.r||0,'mm','min="0" max="200"');
    if(c.t!=='n'){
  ...
  box.querySelectorAll('.kfg_cutrow').forEach(row=>{
    ...
    const uebernehmen=(el)=>{
      ...
      if(f==='e'){ c.e=Math.max(0,Math.min(3,v)); }
      ...
      if(c.t==='n'){ /* Ausklinkung an der Kante halten */
        const len=(c.e%2===0)?d.w:d.h;
        c.w=Math.min(c.w,len); c.a=Math.max(0,Math.min(len-c.w,c.a));
      } else if(c.t!=='p'){
        const w=c.t==='c'?c.d:c.w, h=c.t==='c'?c.d:c.h;
        c.cx=Math.max(w/2,Math.min(d.w-w/2,c.cx));
        c.cy=Math.max(h/2,Math.min(d.h-h/2,c.cy));
      }
      render();
    };
...
$('addNotch').addEventListener('click',addNotch);
...
(function(){
  const l=$('knL'), w=$('knW');
  const upd=()=>{
    const d=dims(), max=S.kanal.pos==='quer'?d.h:d.w;
    S.kanal.l=Math.max(10,Math.min(+l.value||0,max));
    S.kanal.w=Math.max(20,Math.min(+w.value||60,200));
    const rg=$('knRange'); if(rg) rg.textContent=`max. ${max} cm`;
    render();
  };
  let t; [l,w].forEach(el=>el&&el.addEventListener('input',()=>{clearTimeout(t);t=setTimeout(upd,250)}));
  document.querySelectorAll('#knPos .kfg_chip').forEach(b=>b.addEventListener('click',()=>{
    S.kanal.pos=b.dataset.kp;
    document.querySelectorAll('#knPos .kfg_chip').forEach(x=>x.classList.toggle('is-active',x===b));
    upd();
  }));
})();
...
document.querySelectorAll('.kfg_check input').forEach(i=>i.addEventListener('change',()=>{
  ...
  if(i.dataset.x==='kanal')$('kanalBlock').classList.toggle('is-open',i.checked);
  render();
}));
...
function openOrder(){
  ...
  const cutsTxt=(G?S.cuts:[]).map(c2=>{
    const f=v=>(''+(Math.round(v*10)/10)).replace('.',',');
    if(c2.t==='n') return `Ausklinkung ${f(c2.w)} × ${f(c2.dp)} cm an der Kante ${KANTEN_NAMEN[c2.e]}, `
      +`${f(c2.a)} cm vom ${['linken','hinteren','rechten','vorderen'][c2.e]} Anfang, Innenradius R${c2.r||0}`;
  ...
  if(S.extras.kanal)presets.push(`Kabelkanal ${S.kanal.l} cm × ${S.kanal.w} mm (${{mitte:'längs mittig',hinten:'an der Hinterkante',quer:'quer mittig'}[S.kanal.pos]}), von unten gefräst`);
  ...
  const props={...(hitP?{'_kfg_variant_id':hitP[1],'_kfg_sku':hitP[2]||''}:{}),
    ...
    ...(S.extras.kanal?{'_kfg_kabelkanal':`${S.kanal.l}cm x ${S.kanal.w}mm ${S.kanal.pos}`}:{}),
...
function cutLen(c){
  ...
  if(c.t==='n'){
    const len=(c.e%2===0)?dims().w:dims().h;
    const buendig=(c.a<=0.05?1:0)+((c.a+c.w)>=len-0.05?1:0);
    return (c.w||0) + (2-buendig)*(c.dp||0);      /* buendige Seite wird nicht geschnitten */
  }
...
function cutPrice(c){
  ...
  const P = c.t==='n' ? NOTCH_PRICE : FREI_PRICE;
  return Math.round((P.basis + P.lfm*cutLen(c)/100)*10)/10;   /* auf 10 Cent */
}
...
function clampCuts(){
  ...
  S.cuts.forEach(c=>{
    if(c.t==='n'){
      const len=(c.e%2===0)?d.w:d.h, tief=(c.e%2===0)?d.h:d.w;
      c.w=Math.max(1,Math.min(c.w,len));
      c.dp=Math.max(0.5,Math.min(c.dp,tief*0.9));
      c.a=Math.max(0,Math.min(len-c.w,c.a));
      c.r=Math.max(0,Math.min(c.r||0,Math.min(c.w,c.dp)*5));   /* halbe kleinste Seite, in mm */
    } else if(c.t!=='p'){
      const w=c.t==='c'?c.d:c.w, h=c.t==='c'?c.d:c.h;
      c.cx=Math.max(w/2,Math.min(Math.max(w/2,d.w-w/2),c.cx));
      c.cy=Math.max(h/2,Math.min(Math.max(h/2,d.h-h/2),c.cy));
    }
  });
}
function cutWarn(c){
  ...
  if(c.t==='n'){
    const len=(c.e%2===0)?d.w:d.h, tief=(c.e%2===0)?d.h:d.w;
    if(c.w>len+0.05) return 'Breiter als die Kante (' + len + ' cm)';
    if(c.w>=len-0.05&&c.dp>=tief-0.05) return 'Ausklinkung so groß wie die Platte';
    if(c.dp>tief*0.6) return 'Tiefer als 60 % der Platte — bitte vorher anfragen';
    if(c.r>0&&c.r<5) return 'Innenradius mindestens R5 (Fräserdurchmesser)';
    return '';
  }
  if(c.t==='p') return '';
  const w=c.t==='c'?c.d:c.w, h=c.t==='c'?c.d:c.h;
...
function cutMass(c){
  ...
  if(c.t==='n') return z(c.w)+' × '+z(c.dp)+' cm · Kante '+KANTEN_NAMEN[c.e];
  if(c.t==='p') return (c.pts||[]).length+' Punkte';
...
function cutTypName(c){
  ...
  return {r:'Ausschnitt', c:'Runder Ausschnitt', n:'Ausklinkung', p:'Freie Kontur'}[c.t]||'Bearbeitung';
}
...
function updateSticky(){
  ...
  if(window.innerWidth<980){
    ...
    const cap=Math.round(window.innerHeight*0.34)+'px';
  ...
  const stufe = n => {
    ...
    if(det && !det.dataset.manual) setDetailAuto(det, n<3);
  };
...
/* Am Desktop bewusst KEINE eingeblendete Preisleiste (Sascha 27.07.) — auf
   Mobil bleibt die feste Leiste unten wie gehabt. */
