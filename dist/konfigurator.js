  ...
  if(S.form!=='round'){
    ...
    all.forEach((c2,i)=>{
      ...
      if(c2.t==='k'){
        ...
        const mp=kp[Math.floor(kp.length/2)];
        inner+=`<text class="dim-text" x="${mp[0]}" y="${mp[1]-bw/2-6}" text-anchor="middle"
...
function polyKlick(px,py){
  ...
  if(!polyTmp){ polyTmp=[[cx,cy]];
    toast(S.draw==='k'?'Weitere Punkte klicken · Doppelklick beendet den Kanal'
                      :'Weitere Punkte klicken · Doppelklick schließt die Kontur');
    drawStage(); return; }
  const erst=polyTmp[0];
  if(S.draw!=='k' && polyTmp.length>=3 && Math.hypot(cx-erst[0],cy-erst[1])<Math.max(2,G.w*0.02)){ polySchliessen(); return; }
...
function kanalAbschliessen(){
  if(!polyTmp || polyTmp.length<2){ polyTmp=null; setDraw(null); drawStage(); return; }
  const xs=polyTmp.map(p=>p[0]), ys=polyTmp.map(p=>p[1]);
  const cx=(Math.min(...xs)+Math.max(...xs))/2, cy=(Math.min(...ys)+Math.max(...ys))/2;
  const d=dims();
  /* Enden automatisch als offen erkennen, wenn sie nah an der Kante geklickt wurden */
  const amRand=p=>p[0]<2||p[1]<2||p[0]>d.w-2||p[1]>d.h-2;
  const en=(amRand(polyTmp[0])?'a':'')+(amRand(polyTmp[polyTmp.length-1])?'e':'');
  const c={t:'k', cx, cy, w:60, dp:10, seite:'unten', enden:en||'zu',
           pts:polyTmp.map(p=>[p[0]-cx,p[1]-cy])};
  c.w=Math.round(c.w); c.h=Math.max(...ys)-Math.min(...ys);
  polyTmp=null; setDraw(null);
  if(cutLen(c)<5){ toast('Zu kurz: mindestens 5 cm'); render(); return; }
  S.cuts.push(c);
  toast('Kabelkanal hinzugefügt · '+fmt(cutPrice(c)));
  render();
}
/* Offene Enden bis an die naechste Kante ziehen */
function kanalPunkte(c){
  const d=dims(), p=polyAbs(c).map(q=>[q[0],q[1]]);
...
function addCutHoles(sh){
  ...
  S.cuts.forEach(c2=>{
    if(c2.t==='n'||c2.t==='k') return;           /* Kontur bzw. eigene Geometrie */
...
function buildCutList(){
  ...
  box.querySelectorAll('.kfg_cutrow').forEach(row=>{
    ...
    const uebernehmen=(el)=>{
      ...
      else if(f==='e'){ c.e=Math.max(0,Math.min(3,v)); }
...
function startDraw(e){ if(!G)return;
  ...
  if(S.draw==='p'||S.draw==='k'){ const [px,py]=svgPt(e); polyKlick(px,py); e.preventDefault(); return; }
  if(tmpCut)return;
  const [px,py]=svgPt(e); tmpCut={t:S.draw,x0:(px-G.x)/G.sc,y0:(py-G.y)/G.sc,x1:null,y1:null};
  e.preventDefault(); }
$('stage').addEventListener('dblclick',e=>{
  if(S.draw==='p'){ e.preventDefault(); polySchliessen(); }
  if(S.draw==='k'){ e.preventDefault(); kanalAbschliessen(); }
});
window.addEventListener('keydown',e=>{
  if(S.draw!=='p'&&S.draw!=='k') return;
  if(e.key==='Enter'){ e.preventDefault(); S.draw==='k'?kanalAbschliessen():polySchliessen(); }
...
function endDraw(){
  ...
  if(S.draw==='p'||S.draw==='k') return;        /* Polygon und Kanal enden per Doppelklick */
  if(!S.draw||!tmpCut)return;
...
function cutLen(c){
  ...
  if(c.t==='k'&&c.pts&&c.pts.length>1){        /* offener Zug, nicht geschlossen */
...
function cutBox(c){
  ...
  if(c.t==='p'||c.t==='k'){
    const q=(c.pts||[[0,0]]);
    const bw=Math.max(...q.map(p=>p[0]))-Math.min(...q.map(p=>p[0]));
    const bh=Math.max(...q.map(p=>p[1]))-Math.min(...q.map(p=>p[1]));
    const z=c.t==='k'?(c.w||60)/10:0;
    return {w:bw+z, h:bh+z};
  }
  return {w:c.w||0, h:c.h||0};
}
function cutTypName(c){
  if(c.preset) return PRESETS[c.preset].label;
  return {r:'Ausschnitt', c:'Runder Ausschnitt', n:'Ausklinkung', p:'Freie Kontur', k:'Kabelkanal'}[c.t]||'Bearbeitung';
}
