...
var FALLBACK_BASE = 'https://cdn.jsdelivr.net/gh/SaschaKesslerPro/kessler-pro-scripts@5735e20';
  var BASE = (function(){
    try{
...
function cornerOuterCount(){ return S.form==='lform' ? 5 : S.form==='bauch' ? 6 : 4; }
...
const S = { mat:'dekor', dekor:'buk', mpxSurface:'natur', absColor:'dekor',
            ...
            bs:{L:200, BR:90, a:55, b:55, c:60, t:15, w1:BS_DEF_W, w2:BS_DEF_W, mittig:false, treiber:'b'},
...
function drawStage(){
  ...
  if(S.form==='round'){
    ...
    inner+=`<path d="${pd}" fill="none" stroke="${edgeCol(S.edges[0])}" stroke-width="5" stroke-linejoin="round"/>`;
    inner+=`<path d="${pd}" fill="none" stroke="#00000018"/>`;
    inner+=dimH(x,x+pw,y+ph+30,bg.L+' cm')+dimV(x+pw+30,y,y+ph,bg.BR+' cm');
    const zz=v=>(''+(Math.round(v*10)/10)).replace('.',',');
    /* Masskette mit der Zahl OBERHALB der Linie — A und B liegen auf der Platte,
       dort ist unter der Linie kein Platz. */
    /* seite = wohin die Zahl rueckt: 'l' ans linke, 'r' ans rechte Ende. Mittig
       stiess sie bei kurzem A oder B mit dem Winkelbogen zusammen, der an der
       Ausschnittecke haengt (gesehen bei A 40 / 120 Grad, 10.09.). */
    const dimHo=(x1,x2,yy,t,ab,seite)=>{
      const rand=Math.max(6,_dimFS*0.4);
      const tx=seite==='l'?x1+rand : seite==='r'?x2-rand : (x1+x2)/2;
      const anker=seite==='l'?'start' : seite==='r'?'end' : 'middle';
      return `<line class="dim-line" x1="${x1}" y1="${yy}" x2="${x2}" y2="${yy}"/>`
      +`<line class="dim-line" x1="${x1}" y1="${yy-4}" x2="${x1}" y2="${yy+4}"/>`
      +`<line class="dim-line" x1="${x2}" y1="${yy-4}" x2="${x2}" y2="${yy+4}"/>`
      +`<text class="dim-text" x="${tx}" y="${yy-(ab||6)}" text-anchor="${anker}">${t}</text>`; };
    const yF=y+ph, yI=y+(bg.BR-bg.t)*sc, tief=yF-yI;
    const xa1=x+bg.a*sc, xb0=x+(bg.L-bg.b)*sc;
    const xc0=x+(bg.a+bg.r1)*sc, xc1=x+(bg.L-bg.b-bg.r2)*sc;
    /* Auf dem Handy ist die Buehne klein und die Masszahl darum GROSS (dimSkala).
       Alle Platzfragen haengen deshalb an _dimFS, nicht an festen Pixeln — sonst
       schoben sich A, C, die Tiefe und die Winkel uebereinander. */
    const luft=_dimFS;
    const kY=yF-Math.max(15,luft*0.55), kA=Math.max(6,luft*0.3);
    if(xa1-x>luft*3.4) inner+=dimHo(x,xa1,kY,'A '+zz(bg.a),kA,'l');
    if(x+pw-xb0>luft*3.4) inner+=dimHo(xb0,x+pw,kY,'B '+zz(bg.b),kA,'r');
    /* C und Tiefe stehen zusammen IM Ausschnitt — eine Zeile, damit sie sich nie
       gegenseitig ueberschreiben. Passt der Ausschnitt nicht, wandert die Zeile
       ueber die Platte, damit keine Angabe verlorengeht. */
    const cText=`C ${zz(bg.c)} · Tiefe ${zz(bg.t)}`;
    if((xc1-xc0)>luft*8 && tief>luft*1.7){
      inner+=`<line class="dim-line" x1="${xc0}" y1="${yI+10}" x2="${xc1}" y2="${yI+10}"/>`
        +`<line class="dim-line" x1="${xc0}" y1="${yI+6}" x2="${xc0}" y2="${yI+14}"/>`
        +`<line class="dim-line" x1="${xc1}" y1="${yI+6}" x2="${xc1}" y2="${yI+14}"/>`
        +`<text class="dim-text" x="${(xc0+xc1)/2}" y="${Math.min(yI+10+16, yF-4)}" text-anchor="middle">${cText}</text>`;
    } else {
      /* Kein Platz im Ausschnitt: die Zeile steht auf der Platte direkt darueber. */
      inner+=`<text class="dim-text" x="${x+pw/2}" y="${Math.max(y+luft, yI-luft*1.4)}" text-anchor="middle">Ausschnitt ${cText} cm</text>`;
    }
    /* Winkelboegen an den beiden aeusseren Ausschnittecken — dort sieht der Kunde
       den Winkel zwischen Vorderkante und Schraege, derselbe Wert wie am Ende von C. */
    const bogenPlatz=tief>luft*1.5;
    if(bogenPlatz&&bg.w1>90.5&&bg.a>1&&xa1-x>luft*2)
      inner+=winkelBogen(ptsL[6],ptsL[7],ptsL[5],Math.max(12,Math.min(28,bg.t*sc*0.45,bg.a*sc*0.45)),bg.w1+'°');
    if(bogenPlatz&&bg.w2>90.5&&bg.b>1&&x+pw-xb0>luft*2)
      inner+=winkelBogen(ptsL[3],ptsL[2],ptsL[4],Math.max(12,Math.min(28,bg.t*sc*0.45,bg.b*sc*0.45)),bg.w2+'°');
    if(cornerCount()>0) inner+=`<text class="dim-text" x="${x+8}" y="${y-10}">Ecken: ${cornerLabel()}</text>`;
...
function bsAbgeleitet(){
  /* Welches Feld folgt aus den anderen? */
  ...
  const p=new URLSearchParams({m:S.mat,d:S.dekor,f:S.form,t:S.thick,c:S.corner,cr:S.cornerR.join('-'),er:S.edgeR,sf:S.mpxSurface,ac:S.absColor,
    ...
    ...(S.form==='round'?{dm:S.D}
       ...
       :S.form==='bauch'?(function(){ const g=bsGeo(), r=v=>Math.round(v*10)/10;
          return {bs:[g.L,g.BR,r(g.a),r(g.b),g.c,r(g.t),g.w1,g.w2,S.bs.mittig?1:0,S.bs.treiber].join(','),
...
function openOrder(){
  ...
  const mass=S.form==='round'?`Ø ${S.D} cm`
    ...
    :S.form==='bauch'?(function(){ const g=bsGeo(), z=v=>(''+(Math.round(v*10)/10)).replace('.',',');
        return `${g.L} × ${g.BR} cm · Bauchausschnitt A ${z(g.a)} · C ${z(g.c)} · B ${z(g.b)} · Tiefe ${z(g.t)} cm · Winkel ${g.w1}°/${g.w2}°`
  ...
  const props={...(hitP?{'_kfg_variant_id':hitP[1],'_kfg_sku':hitP[2]||''}:{}),
    ...
    ...(S.form==='bauch'?(function(){ const g=bsGeo(), r=v=>Math.round(v*10)/10;
      return {'_kfg_bauch':`${r(g.a)}|${r(g.b)}|${g.c}|${r(g.t)}|${g.w1}|${g.w2}|${S.bs.mittig?'mittig':'frei'}`,
