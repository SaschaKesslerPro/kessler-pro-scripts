...
/* Abstand Linie -> Zahl: halbe Schrifthoehe plus 6 Einheiten Luft. Waechst
   damit automatisch mit, wenn die Schrift auf kleinen Schirmen groesser
   gerechnet wird. */
function dimAb(){ return Math.round(_dimFS*0.62)+6; }
...
var DIM_ZIEL = 15;                      /* so gross soll die Zahl WIRKLICH sein */
...
function dimSkala(){
  ...
  const ziel=window.innerWidth<980 ? DIM_ZIEL-1 : DIM_ZIEL;
  _dimFS = Math.round(ziel/m*10)/10;
  st.style.setProperty('--dim-fs', _dimFS+'px');
  st.style.setProperty('--dim-halo', Math.round(3/m*10)/10+'px');
...
(function(){
  const imBild=()=>{
    const r=$('stickyCol'); if(!r) return false;
    const b=r.getBoundingClientRect();
    return b.top < window.innerHeight*0.6 && b.bottom > 0;
  };
  let tick=false;
  addEventListener('scroll',()=>{
    ...
    requestAnimationFrame(()=>{
      ...
      updateBottomBar();
      if(!imBild() || y<80){ kopfleisteZeigen(true); return; }
...
function updateBottomBar(){
  ...
  const kol=$('stickyCol');
  if(!kol){ bar.classList.remove('is-desk'); return; }
  const r=kol.getBoundingClientRect();
  /* Unterkante der Spalte ueber dem oberen Bildrand = wir sind darunter.
     40 px Puffer, damit die Leiste nicht am Rand flackert. */
  bar.classList.toggle('is-desk', r.bottom < 40);
}
