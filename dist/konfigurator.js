...
function cornerCount(){ return S.form==='rect'&&S.corner>0 ? S.cornerSel.filter(Boolean).length : 0; }
function cornerOn(i){ return S.form==='rect'&&S.corner>0&&!!S.cornerSel[i]; }
/* Radius je Ecke in cm-Einheiten des Aufrufers (mm/10 * Skalierung) */
function cornerRadii(scale, maxA, maxB){
  const r = S.form==='rect' ? S.corner/10*scale : 0;
  const cap = Math.min(maxA, maxB);
  return [0,1,2,3].map(i => cornerOn(i) ? Math.min(r, cap) : 0);
}
...
const S = { mat:'dekor', dekor:'buk', mpxSurface:'natur', absColor:'dekor',
            ...
            corner:0, cornerSel:[true,true,true,true], edgeR:3, edges:['abs','abs','abs','abs'],
...
function calc(){
  ...
  const ecken=(CORNER_PRICE[S.corner]||4.9)*cornerCount();
...
function drawStage(){
  ...
  if(S.form==='round'){
    ...
    if(cornerCount()>0) inner+=`<text class="dim-text" x="${x+8}" y="${y-10}">${
      cornerCount()===4?`Ecken R${S.corner}`:`R${S.corner} · ${cornerCount()}× (${S.cornerSel.map((v,i)=>v?CORNER_NAMES[i]:null).filter(Boolean).join(', ')})`}</text>`;
  ...
  svg.querySelectorAll('.kfg_cornerhit').forEach(e=>e.addEventListener('click',ev=>{
    ev.stopPropagation();
    const i=+e.dataset.c;
    if(S.corner===0){                       /* noch kein Radius gewaehlt: sinnvollen setzen */
      S.corner=rules().minCorner>0?rules().minCorner:10;
      S.cornerSel=[false,false,false,false];
    }
    S.cornerSel[i]=!S.cornerSel[i];
    if(!S.cornerSel.some(Boolean)) S.corner=0;
    buildCorner(); render();
    toast(S.cornerSel[i]?`Ecke ${CORNER_NAMES[i]} abgerundet (R${S.corner})`:`Ecke ${CORNER_NAMES[i]} wieder eckig`);
  }));
...
function drawDetail(){
  ...
  $('detailLabel').innerHTML=`<b>${c.dekorName}</b>
    ...
    ${cornerCount()>0?`<span>Ecken: R${S.corner} · ${cornerCount()===4?'alle vier':S.cornerSel.map((v,i)=>v?CORNER_NAMES[i]:null).filter(Boolean).join(', ')}</span>`:''}
...
function plateShape(){
  ...
  const RC=[3,2,1,0].map(i=>cornerOn(i)?Math.min(S.corner/100,cap):0);
  const x=-w/2,y=-h/2;
...
function buildMats(){
  ...
  $('matGrid').querySelectorAll('.kfg_mat').forEach(b=>b.addEventListener('click',()=>{
    ...
    if(S.mat==='dekor'&&cornerCount()>0&&S.corner<rules().minCorner) S.corner=rules().minCorner;
...
function buildCorner(){
  ...
  $('cornerChips').querySelectorAll('.kfg_chip').forEach(b=>b.addEventListener('click',()=>{
    ...
    /* Radius gewaehlt, aber keine Ecke aktiv -> alle vier annehmen */
    if(S.corner>0&&!S.cornerSel.some(Boolean)) S.cornerSel=[true,true,true,true];
    buildCorner(); render();
  }));
  buildCornerSel();
...
function buildCornerSel(){
  ...
  const show=S.form==='rect'&&S.corner>0;
  box.style.display=show?'block':'none';
  if(!show) return;
  el.innerHTML=[0,1,2,3].map(i=>
    `<button class="kfg_chip${S.cornerSel[i]?' is-active':''}" data-cs="${i}" aria-pressed="${!!S.cornerSel[i]}">`
    +`${cornerIcon(i,!!S.cornerSel[i])}${CORNER_NAMES[i]}</button>`).join('')
    +`<button class="kfg_chip" data-cs="all">${cornerCount()===4?'Keine':'Alle vier'}</button>`;
  el.querySelectorAll('.kfg_chip').forEach(b=>b.addEventListener('click',()=>{
    if(b.dataset.cs==='all'){ const all=cornerCount()===4; S.cornerSel=[0,1,2,3].map(()=>!all); }
    else { const i=+b.dataset.cs; S.cornerSel[i]=!S.cornerSel[i]; }
    if(!S.cornerSel.some(Boolean)){ S.corner=0; buildCorner(); }
    else buildCornerSel();
    render();
  }));
}
...
function validate(){
  ...
  if(S.form==='rect'&&S.mat==='dekor'&&cornerCount()>0&&S.corner<r.minCorner){
...
function render(){
  ...
  if(c.ecken>0)rows.push([`Eckenrundung R${S.corner} (${cornerCount()}× à ${fmt(CORNER_PRICE[S.corner]||4.9)})`,c.ecken]);
  if(c.extras>0){
...
function restoreFromHash(){
  ...
  if(/^[01]{4}$/.test(g('cs')||'')) S.cornerSel=g('cs').split('').map(v=>v==='1');
  ...
  const ic=$('inC'); if(ic&&[0,30,50,100].indexOf(S.corner)<0&&S.corner>0) ic.value=S.corner;
}
function syncURL(){
  const p=new URLSearchParams({m:S.mat,d:S.dekor,f:S.form,t:S.thick,c:S.corner,cs:S.cornerSel.map(v=>v?1:0).join(''),er:S.edgeR,sf:S.mpxSurface,ac:S.absColor,
...
let cT;$('inC').addEventListener('input',()=>{clearTimeout(cT);cT=setTimeout(()=>{
  ...
  if(v>0&&!S.cornerSel.some(Boolean)) S.cornerSel=[true,true,true,true];
...
function openOrder(){
  ...
  const rows=[
    ...
    ...(cornerCount()>0?[['Eckenradius',`R${S.corner} an ${cornerCount()} Ecke${cornerCount()>1?'n':''}: `
      +S.cornerSel.map((v,i)=>v?CORNER_NAMES[i]:null).filter(Boolean).join(', ')]]:[]),
  ...
  const props={...(hitP?{'_kfg_variant_id':hitP[1],'_kfg_sku':hitP[2]||''}:{}),
    ...
    ...(cornerCount()>0?{'_kfg_eckenradius_mm':S.corner,
      '_kfg_ecken':S.cornerSel.map((v,i)=>v?CORNER_NAMES[i]:null).filter(Boolean).join(' | ')}:{}),
