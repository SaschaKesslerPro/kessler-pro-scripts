...
function normEdges(){
  ...
  S.edges=S.edges.map(e=>{ e=al[e]||e; return ids.indexOf(e)>=0?e:ids[0]; });
}
...
function buildDekore(){
  ...
  $('dekorGrid').innerHTML=list.map(([k,n])=>
    `<button class="kfg_dekor${k===S.dekor?' is-active':''}" data-d="${k}" title="${n}"><span class="sw" style="background-image:url(${TEX_THUMB[k]})"></span><span>${n}</span></button>`).join('');
...
function buildEdges(){
  ...
  $('edgeChips').innerHTML=EDGEPROFILES[S.mat].map(p=>
    `<button class="kfg_chip${S.edges.every(e=>e===p[0])?' is-active':''}" data-edge="${p[0]}">${p[1]} <small>${edgeNote(p)}</small></button>`).join('');
...
function buildMassband(){
  ...
  el.innerHTML=MASSBAND.map(([k,n,note])=>
    `<button class="kfg_chip${S.massband===k?' is-active':''}" data-mb="${k}">${n}${note?` <small>${note}</small>`:''}</button>`).join('');
  el.querySelectorAll('.kfg_chip').forEach(b=>b.addEventListener('click',()=>{
...
function buildConf(){
  ...
  if(S.mat==='szwal'&&S.machine) items.push({t:`Maschine: ${S.machine}`, s:5});
...
function knoepfeSperren(text){
  const alt=$('cta').textContent, altB=($('ctaBuy')||{}).textContent;
  [$('cta'),$('ctaBar'),$('ctaBuy')].forEach(b=>{ if(b){ b.disabled=true; } });
  $('cta').textContent=text; $('ctaBar').textContent=text;
  return ()=>{ _checkoutLaeuft=false; [$('cta'),$('ctaBar'),$('ctaBuy')].forEach(b=>{ if(b) b.disabled=false; }); $('cta').textContent=alt; $('ctaBar').textContent=alt; if($('ctaBuy')) $('ctaBuy').textContent=altB; };
}
async function inWarenkorbLegen(variantId, attribute){
  const sf=shopyflow(); if(!sf) throw new Error('kein Warenkorb');
  await sf.addToCart({ lineItems:[{ merchandiseId: variantId, quantity:1, attributes: attribute||[] }], useShopifyId:true });
...
function sofortkaufStarten(){
  ...
  fetch(WARENKORB_URL,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(bodyFuerWorker({sofort:true})),signal:ctl.signal})
    .then(r=>r.json().then(d=>({ok:r.ok,d})))
    .then(({ok,d})=>{ clearTimeout(tm); if(ok && d && d.checkoutUrl){ location.href=d.checkoutUrl; return; } throw new Error((d&&d.fehler)||'kein checkoutUrl'); })
...
if($('ctaBuy')) $('ctaBuy').addEventListener('click',()=>{ const hit=nurLager()&&shopHit(); if(hit||kannBezahlen()) sofortkaufStarten(); else anfrageMail(); });
