...
function needsOffer(){ return S.extras.custom||S.extras.kanal||isLack()||S.cuts.some(c=>!c.preset); }
function delivDate(){
...
function extrasList(){
  ...
  const frei=S.cuts.filter(c=>!c.preset).length;   /* Preset-Ausschnitte stecken auch in S.cuts */
  if(frei) out.push({t:`${frei}× eigener Ausschnitt`, s:5, warn:true});
...
/* Hoehe des sticky/fixed Site-Headers messen — Webflow-Header, Ankuendigungs-
   ...
   (Scroll-Header, Sprachumschalter). Darum bei jedem Aufruf neu bestimmen. */
function headerBottom(){
  let hb=0;
  const kandidaten=document.querySelectorAll('header, [class*="header"], [class*="navbar"], [class*="nav_"]');
...
function updateSticky(){
  ...
  el.style.top = top+'px';
  el.classList.remove('is-tight','is-tighter','is-scroll');
  if(det && !det.dataset.manual) setDetailAuto(det, true);   /* erst mal alles offen */
  setMax('');
  /* Draufsicht so weit verkleinern, dass die ganze Spalte passt — aber nie
     unter die uebergebene Mindesthoehe, sonst wird die Platte zur Briefmarke. */
  const passt = min => {
    if(el.offsetHeight<=platz) return true;
    if(!stage) return false;
    const drumherum = el.offsetHeight - stage.getBoundingClientRect().height;
    const maxStage = Math.round(platz - drumherum);
    if(maxStage >= min){ setMax(maxStage+'px'); return true; }
    return false;
  };
  /* Reihenfolge der Opfer: erst die Draufsicht kleiner, dann Vertrauenszeile und
     Teilen-Buttons, dann die Detailkarte (mit sichtbarer Aufklapp-Zeile, sie
     laesst sich also jederzeit zurueckholen), dann Chips und Aufschluesselung.
     Notfall: die Spalte scrollt intern — Preis und CTA bleiben erreichbar. */
  if(!passt(300)){
    el.classList.add('is-tight');            /* Vertrauenszeile, Teilen, kleineres Kantenbild */
    if(!passt(260)){
      el.classList.add('is-tighter');        /* Chips + Aufschluesselung */
      if(!passt(220)){
        if(det && !det.dataset.manual) setDetailAuto(det, false);
        if(!passt(180)){ setMax('180px'); el.classList.add('is-scroll'); }
      }
    }
  }
}
