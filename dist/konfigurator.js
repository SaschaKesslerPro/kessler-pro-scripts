...
/* Punkte A und B der Schraege (Senior 03.09. Skizze, 07.09. praezisiert):
   A = Punkt auf der inneren Kante der Ausklinkung, "in der Mitte der Platte";
   von der Plattenkante bis A ist der Schnitt gerade, ab A laeuft die Schraege
   bis B = Ecke der Ausklinkung an der Aussenkante. sb = Abstand von A zur
   Plattenkante (cm, entlang der Tiefe); 0 = A liegt auf der Kante, die Schraege
   geht durch. Winkel = der Winkel BEI A zwischen geradem Schnitt und Schraege,
   im Ausschnitt gemessen: 180 Grad = gerade, 90 + atan((ah - sb) / aw). */
...
function winkelBogen(A,K,Bp,r,text){
  ...
  return `<path d="M${f(p1[0])} ${f(p1[1])} A${f(r)} ${f(r)} 0 0 ${sweep} ${f(p2[0])} ${f(p2[1])}" fill="none" stroke="#1E1E1E" stroke-width="1.2"/>`
    +`<text class="dim-text" x="${f(A[0]+bx*(r+13))}" y="${f(A[1]+by*(r+13)+4)}" text-anchor="middle">${text}</text>`;
}
function lfPts(){
...
function drawStage(){
  ...
  if(S.form==='round'){
    ...
    /* Schraege mit Punkt A: A innen auf der Ausklinkungskante, B an der Aussenkante,
       Winkelbogen bei A im Ausschnitt zwischen geradem Schnitt und Schraege */
    if(mitA){
      const mx=links, my=vorn, P=([px2,py2])=>[x+(mx?lg.L-px2:px2)*sc, y+(my?lg.B-py2:py2)*sc];
      const A=P([lg.L-lg.aw,lg.sb]), Bp=P([lg.L,lg.ah]), K=P([lg.L-lg.aw,0]);
      inner+=`<circle cx="${A[0]}" cy="${A[1]}" r="3.5" fill="#1E1E1E"/><text class="dim-text" x="${A[0]+(links?9:-9)}" y="${A[1]+(vorn?12:-6)}" text-anchor="middle">A</text>`;
      inner+=`<circle cx="${Bp[0]}" cy="${Bp[1]}" r="3.5" fill="#1E1E1E"/><text class="dim-text" x="${Bp[0]+(links?-9:9)}" y="${Bp[1]+(vorn?-6:12)}" text-anchor="middle">B</text>`;
      inner+=winkelBogen(A,K,Bp,Math.min(22,lg.sb*sc*0.8,lg.aw*sc*0.6),`${lfGeo().winkel}°`);
    }
...
function render(){
  ...
  if(c.ecken>0){
    ...
    const auto=na>0?`A/B R${lfMinR()*10} (Fertigungsregel)`:'';
