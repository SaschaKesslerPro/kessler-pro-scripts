...
function stageTex(k){ return _texOk[k] ? TEX[k] : (TEX_THUMB[k] || TEX[k]); }
...
function cornerLimit(i){
  if(S.form==='lform'){
    const g=lfPts(), k=g.ord.indexOf(i); if(k<0) return 0;
...
/* Punkte A und B der Schraege (Senior 03.09. Skizze, 07.09. zweimal praezisiert):
   B = Ecke der Ausklinkung AN DER PLATTENKANTE (dort, wo die Ausklinkung offen
   ist); A = Punkt auf der inneren Kante der Ausklinkung, "in der Mitte der
   Platte", sb cm von B entfernt. Von B bis A ist der Schnitt gerade, ab A laeuft
   die Schraege bis zur Aussenkante (dieser Endpunkt hat keinen Buchstaben).
   sb = 0: A liegt auf B, die Schraege geht bis zur Kante durch.
   Winkel = der Winkel BEI A zwischen A-B (gerade) und der Schraege, im
   Ausschnitt gemessen: 180 Grad = gerade, 90 + atan((ah - sb) / aw). */
...
const S = { mat:'dekor', dekor:'buk', mpxSurface:'natur', absColor:'dekor',
            form:'rect', L:120, B:60, D:80, lf:{L:180,B:120,aw:90,ah:60,pos:null,schnitt:'gerade',sb:0}, thick:'25',
...
function edgeNote(p){
  if(EDGEPROFILES[S.mat]!==EDGE_MPX || p[0]==='abs') return p[3];
  const v=edgeLfm(p[0]);
  return v>0 ? `+ ${v} €/lfm` : 'serienmäßig';
}
...
function drawStage(){
  ...
  const PADL=S.form==='lform'?rand:16, PADR=rand, PADB=rand;
  ...
  if(S.form==='round'){
    ...
    inner+=`<image href="${tex}" x="${cx-r*1.16}" y="${cy-r*1.16}" width="${2.32*r}" height="${2.32*r}" preserveAspectRatio="xMidYMid slice" clip-path="url(#plateClip)"/>`;
    ...
    const mitA=lfSchraeg()&&lg.sb>0;
    /* Winkel steht bei A im Ausschnitt (Senior 07.09.); ohne A (Schraege bis zur Kante) im Mass der Ausklinkung */
    const ntxt=lfSchraeg()&&!mitA?`${lg.aw} × ${lg.ah} · ${lfGeo().winkel}°`:`${lg.aw} × ${lg.ah}`;
    /* Schraege mit Punkt A: B = Ecke der Ausklinkung an der Plattenkante, A innen auf
       der Ausklinkungskante (sb von B), Winkelbogen bei A im Ausschnitt zwischen A-B
       und der Schraege; das Ende der Schraege an der Aussenkante bleibt ohne Buchstaben */
    if(mitA){
      const mx=links, my=vorn, P=([px2,py2])=>[x+(mx?lg.L-px2:px2)*sc, y+(my?lg.B-py2:py2)*sc];
      const A=P([lg.L-lg.aw,lg.sb]), Bp=P([lg.L-lg.aw,0]), E=P([lg.L,lg.ah]);
      inner+=`<circle cx="${A[0]}" cy="${A[1]}" r="3.5" fill="#1E1E1E"/><text class="dim-text" x="${A[0]+(links?9:-9)}" y="${A[1]+(vorn?-6:12)}" text-anchor="middle">A</text>`;
      inner+=`<circle cx="${Bp[0]}" cy="${Bp[1]}" r="3.5" fill="#1E1E1E"/><text class="dim-text" x="${Bp[0]+(links?9:-9)}" y="${Bp[1]+(vorn?16:-8)}" text-anchor="middle">B</text>`;
      inner+=winkelBogen(A,Bp,E,Math.min(18,lg.sb*sc*0.6,lg.aw*sc*0.5),`${lfGeo().winkel}°`);
    }
...
/* ── Zeichnen-Werkzeug ── */
let G=null, tmpCut=null, dragCut=null, FORCE_DISTS=false, polyTmp=null, _imRender=false;
...
function draw3D(){
  ensure3D(()=>{
    ...
    if(isLack()) topMat=new THREE.MeshStandardMaterial({color:0xdcd9d2,roughness:.5});
      ...
      let t=three.tex[key];
      if(!t){ t=new THREE.TextureLoader().load(TEX[key],frame3D);
        t.encoding=THREE.sRGBEncoding;
        t.wrapS=t.wrapT=THREE.ClampToEdgeWrapping; t.anisotropy=8; t.kfgCache=true;   /* r128 kennt kein userData an Texturen */
        three.tex[key]=t; }
...
function buildMats(){
  $('matGrid').innerHTML=Object.entries(MATERIALS).map(([k,m])=>
    `<button class="kfg_mat${k===S.mat?' is-active':''}" aria-pressed="${k===S.mat}" data-m="${k}"><b>${m.name}</b><small>${m.sub}</small></button>`).join('');
...
function buildEdgeExtras(){
  ...
  if(b2){
    ...
    if(pr){ const e=S.edges.find(x=>x!=='abs')||'nicht';
      pr.textContent = LACK_LFM[e]!==undefined ? `${LACK_LFM[e]} €/lfm` : 'auf Anfrage'; }
  }
...
function buildMassband(){
  ...
  el.innerHTML=MASSBAND.map(([k,n,note])=>
    `<button class="kfg_chip${S.massband===k?' is-active':''}" aria-pressed="${S.massband===k}" data-mb="${k}">${n}${note?` <small>${note}</small>`:''}</button>`).join('');
  el.querySelectorAll('.kfg_chip').forEach(b=>b.addEventListener('click',()=>{
...
function buildLfControls(){
  ...
  const note=$('lfInnerNote'); if(note){ const r=lfMinR()*10, abs=S.edges[0]==='abs';
    note.textContent=(lfSchraeg()
      ? `Punkt A und das Ende der Schräge an der Außenkante werden automatisch verrundet: R${r} — ${abs?'ABS-Kante geklebt':'Kante ohne ABS'} (Fertigungsregel).`
...
function buildQuick(){
  if(S.mat==='szwal'||S.form==='lform'){$('quickBlock').style.display='none';$('massHint').textContent='CNC-Fertigung nach Maß';return;}
...
function validate(){
  ...
  if(S.form==='rect'){ chk('fL','errL','rangeL',+S.L,20,r.maxL); chk('fB','errB','rangeB',+S.B,20,r.maxB); }
    ...
    if(lfSchraeg()){ const ah=+S.lf.ah||10, v=Math.round(+S.lf.sb||0); const bad=!(v>=0&&v<=ah-1); $('fLS').classList.toggle('is-error',bad); $('errLS').textContent=bad?`0 bis ${ah-1} cm`:''; if(bad)ok=false; }
  }
...
function massSperre(an){
  const knoepfe=[$('cta'),$('ctaBar'),$('ctaBuy')].filter(Boolean);
...
function render(){
  ...
  if(!_checkoutLaeuft){ $('cta').textContent=t;$('ctaBar').textContent=t; const cb=$('ctaBuy'); if(cb){ cb.textContent='Sofortkauf'; cb.style.display=kaufbar?'':'none'; } }
  ...
  if(c.ecken>0){
    const n=cornerCount(), na=lfAutoEcken();
  ...
  syncStep5();
  _imRender=true;
  try{ buildQuick(); buildLfControls(); drawStage(); drawDetail(); updateMini(); updateSticky(); updateBottomBar(); buildCounts(); buildConf(); buildBarWhat(); syncURL(); }
...
function restoreFromHash(){
  ...
  if(/^\d{1,3}$/.test(g('lsb')||'')) S.lf.sb=+g('lsb');
...
function buildCounts(){
  ...
  set('k2', S.form==='rect'
      ...
      : {round:'Rund',lform:'L-Form'}[S.form]);
...
function cutListAktualisieren(box){
  ...
  box.querySelectorAll('.kfg_cutrow').forEach(row=>{
    ...
    const len=row.querySelector('.kfg_cutlen:not(.breit)'); if(len && c.t==='k') len.textContent=`Länge ${Math.round(cutLen(c))} cm · ${fmt(kanalLfmPreis(c.w,c.dp))} je lfm`;
...
function buildCutList(){
  ...
  const d=dims();
  { const ae=document.activeElement;
    if(ae && box.contains(ae) && /^(INPUT|SELECT)$/.test(ae.tagName) && box.querySelectorAll('.kfg_cutrow').length===S.cuts.length){ cutListAktualisieren(box); return; } }
...
function _syncURLnow(){
  const p=new URLSearchParams({m:S.mat,d:S.dekor,f:S.form,t:S.thick,c:S.corner,cr:S.cornerR.join('-'),er:S.edgeR,sf:S.mpxSurface,ac:S.absColor,
    ...
    ...(S.form==='round'?{dm:S.D}
       :S.form==='lform'?{ll:S.lf.L,lb:S.lf.B,aw:S.lf.aw,ah:S.lf.ah,lp:lfPos(),ls:lfSchraeg()?'s':'g',lsb:lfSb(),lr:[0,1,2,3,4].map(lfCornerR).join('-')}
...
function formKonsistent(){
  let weg=false;
  if(S.form==='round'||S.form==='lform'){ const e0=S.edges[0]; S.edges=[e0,e0,e0,e0]; }
...
{ let wT; const iw=$('inLW'); if(iw) iw.addEventListener('input',()=>{ clearTimeout(wT); wT=setTimeout(()=>{
    /* Winkel bei A (95..175, 180 = gerade): Punkt A folgt aus Breite x tan(Neigung),
       die Tiefe der Ausklinkung bleibt (v1.17.3 — vorher aenderte der Winkel die Tiefe). */
    const w=Math.max(95,Math.min(175,+iw.value||124))-90, aw=+S.lf.aw||1, ah=+S.lf.ah||10;
    S.lf.sb=Math.max(0,Math.min(ah-1, Math.round(ah-aw*Math.tan(w*Math.PI/180))));
    render(); },300); }); }
{ let sT; const is=$('inLS'); if(is) is.addEventListener('input',()=>{ clearTimeout(sT); sT=setTimeout(()=>{
    S.lf.sb=Math.max(0,Math.min((+S.lf.ah||10)-1, Math.round(+is.value||0))); render(); },300); }); }
...
function openOrder(){
  ...
  const mass=S.form==='round'?`Ø ${S.D} cm`
    :S.form==='lform'?`${S.lf.L} × ${S.lf.B} cm · Ausklinkung ${S.lf.aw} × ${S.lf.ah} cm ${(LF_POS.find(p=>p[0]===lfPos())||[])[1]}${lfSchraeg()?` · schräg ${lfGeo().winkel}°${lfSb()?` · A bei ${lfSb()} cm`:''}`:''}`
    :`${S.L} × ${S.B} cm`;
  ...
  const props={...(hitP?{'_kfg_variant_id':hitP[1],'_kfg_sku':hitP[2]||''}:{}),
    ...
    ...(S.form==='lform'?{'_kfg_lform':`${lfPos()}|${lfSchraeg()?'schraeg':'gerade'}|${lfGeo().winkel}|${lfSb()}`}:{}),
...
function knoepfeSperren(text){
  ...
  [$('cta'),$('ctaBar'),$('ctaBuy')].forEach(b=>{ if(b){ b.disabled=true; } });
  $('cta').textContent=t; $('ctaBar').textContent=t;
  panelSperren(true);
  return ()=>{ _checkoutLaeuft=false; panelSperren(false); [$('cta'),$('ctaBar'),$('ctaBuy')].forEach(b=>{ if(b) b.disabled=false; }); $('cta').textContent=alt; $('ctaBar').textContent=alt; if($('ctaBuy')) $('ctaBuy').textContent=altB; };
}
/* Zurueck per Browser-Zurueck aus der Kasse (bfcache): Sperre loesen, Knoepfe neu beschriften */
window.addEventListener('pageshow',e=>{ if(e.persisted){ _checkoutLaeuft=false; panelSperren(false); [$('cta'),$('ctaBar'),$('ctaBuy')].forEach(b=>{ if(b) b.disabled=false; }); try{ render(); }catch(_){} } });
...
$('btnMuster').addEventListener('click',()=>{                 /* Musterbox anfragen */
  ...
  location.href='mailto:shop@kessler-pro.com?subject='+encodeURIComponent('Musterbox-Bestellung')
    +'&body='+encodeURIComponent('Bitte um die Musterbox (4 Dekore, 4,90 € — beim Kauf voll angerechnet).\nWunsch-Dekor: '+dek+'\n\nLieferadresse:\n');
});
...
function updateMini(){
  ...
  const mass=S.form==='round'?`Ø ${S.D} cm`
    :S.form==='lform'?`${S.lf.L} × ${S.lf.B} cm · L-Form`:`${S.L} × ${S.B} cm`;
