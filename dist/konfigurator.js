...
/* Punkt B der Schraege (Senior 03.09., Skizze): Die Schraege laeuft von A — der
   Ecke der Ausklinkung an der Aussenkante — zu einem Punkt B auf der inneren
   Kante der Ausklinkung; von B bis zur Plattenkante ist der Schnitt gerade.
...
function drawStage(){
  ...
  if(S.form==='round'){
    ...
    /* Schraege mit Punkt B: A und B markieren, wie in der Skizze des Seniors */
    if(lfSchraeg()&&lg.sb>0){
      const mx=links, my=vorn, P=([px2,py2])=>[x+(mx?lg.L-px2:px2)*sc, y+(my?lg.B-py2:py2)*sc];
      const A=P([lg.L,lg.ah]), Bp=P([lg.L-lg.aw,lg.sb]);
      ...
      inner+=`<circle cx="${Bp[0]}" cy="${Bp[1]}" r="3.5" fill="#1E1E1E"/><text class="dim-text" x="${Bp[0]+(links?9:-9)}" y="${Bp[1]+(vorn?12:-6)}" text-anchor="middle">B</text>`;
    }
...
{ let wT; const iw=$('inLW'); if(iw) iw.addEventListener('input',()=>{ clearTimeout(wT); wT=setTimeout(()=>{
    /* Winkel zur Plattenkante: Punkt B folgt aus Breite x tan(Winkel), die Tiefe
       der Ausklinkung bleibt (v1.17.3 — vorher aenderte der Winkel die Tiefe). */
    const w=Math.max(5,Math.min(85,+iw.value||34)), aw=+S.lf.aw||1, ah=+S.lf.ah||10;
...
function openOrder(){
  ...
  const mass=S.form==='round'?`Ø ${S.D} cm`
    :S.form==='lform'?`${S.lf.L} × ${S.lf.B} cm · Ausklinkung ${S.lf.aw} × ${S.lf.ah} cm ${(LF_POS.find(p=>p[0]===lfPos())||[])[1]}${lfSchraeg()?` · schräg ${lfGeo().winkel}°${lfSb()?` · B bei ${lfSb()} cm`:''}`:''}`
    :`${S.L} × ${S.B} cm`;
