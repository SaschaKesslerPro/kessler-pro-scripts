...
(function(){ const d=$('detailCard'); if(d) d.addEventListener('toggle',e=>{
  if(e.isTrusted) d.dataset.manual='1';        /* vom Nutzer bedient */
}); })();
function initMini(){
  ...
  const sync=()=>{
    ...
    /* Mini-Leiste sobald die Draufsicht oben angeschnitten wird — auf flachen
       Fenstern passiert das trotz Sticky-Spalte, gerade beim Arbeiten an Ecken. */
    const stg=document.getElementById('stage');
    const ref=(stg&&stg.getBoundingClientRect().height>0?stg:prev).getBoundingClientRect();
    const on = ref.top < hb-4;
...
function updateSticky(){
  ...
  /* Passt die Spalte nicht ins Fenster, lief die Draufsicht oben raus — genau
     beim Arbeiten an den Ecken. Die Detailkarte klappt daher automatisch zu,
  ...
  /* Immer UNTER dem Header beginnen. Vorher wurde der Wert negativ, sobald der
     ...
     die Preisleiste, statt die Draufsicht anzuschneiden. */
  el.style.top=(headerBottom()+12)+'px';
}
/* Untere Preisleiste am Desktop einblenden, sobald die Preiskarte rausscrollt */
function updateBottomBar(){
  const bar=document.querySelector('.kfg_bar'), sum=document.querySelector('.kfg_summary');
  if(!bar||!sum) return;
  if(window.innerWidth<980){ bar.classList.remove('is-desk'); return; }
  const r=sum.getBoundingClientRect();
  bar.classList.toggle('is-desk', r.bottom > window.innerHeight+4);
}
