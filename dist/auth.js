/* kessler-pro-scripts/auth.js v1.0.20 — type-switch + checkbox visuals only (no font overrides) */
(function(){
  if(window.__kpAuthV1)return;
  window.__kpAuthV1=true;

  /* Inject CSS for Webflow FormCheckbox visual override + row layout */
  if(!document.getElementById('kp-auth-css')){
    var css=document.createElement('style');
    css.id='kp-auth-css';
    css.textContent=[
      /* Row layout */
      '.kp-checkbox-row{display:flex !important;align-items:center !important;gap:12px !important;flex-wrap:wrap}',
      ...
      '.kp-checkbox-row .w-checkbox{padding-left:0 !important;margin-bottom:0 !important;margin-right:0 !important;display:flex !important;align-items:center;flex-shrink:0}',
  ...
  /* Click on outer text -> toggle checkbox */
  document.addEventListener('click',function(e){
