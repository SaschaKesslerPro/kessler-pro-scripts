...
- `bilder/dekore/sw-*.jpg` — neun Dekor-Kacheln 160 × 160 aus den Shopfotos (Buche, Ahorn, Kiefer Weiß, Eiche Sonoma, Birke, Weiß, Grau, Weißer/Schwarzer Marmor) für die Dekorreihe im Newsletter.
...
Regeln: 600 px Mails, 400 px Signaturen, Tabellen + Inline-Stile, Bilder extern vom CDN, genau ein Hauptknopf, Rechtszeile und Antwortadresse im Fuß; Kampagnen zusätzlich mit Empfangsgrund und Abmeldelink. Der Worker verschickt nur Transaktionsmails — Kampagnen laufen über Shopify Email oder Klaviyo. **Entschieden 02.09.2026: Kopf B (Ink block).** Umgesetzt in `worker/konfigurator-checkout/src/mail.js` (de/pl/en; Zeichnung als PNG aus `src/bild.js`, PDF im Anhang). Signatur (1–3) und Newsletter-Richtung offen; die Signatur wandert dann in einen Generator (`mail/signatur/`).

Dunkle Sektion (B und N): Linie `rule-on-dark #3A3D41`, Text `text-on-dark #CFCBC3` auf `#17191B` — seit 02.09.2026 in DESIGN.md.
